...
function create_weather_table() 
{
    ...
    try {
        ...
        WHERE CAST(w.zone AS char) <> z.name
			...
			AND z.name NOT LIKE "Dynamis%"
			AND z.name NOT LIKE "Ship%"
			AND z.name NOT LIKE "Spire%"
			AND z.name NOT LIKE "Residential%"
			AND z.name NOT LIKE "Cloister%"
			AND z.name NOT LIKE "Windurst%"
			AND z.name NOT LIKE "Promyvion%"
